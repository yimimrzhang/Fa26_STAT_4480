...
mosaicplot(~ StatusCat96NK_order + TargetB, data = data, 
           main = "Mosaic Plot of StatusCat96NK by TargetB",
           xlab = "StatusCat96NK",
           ylab = "Proportion",
           color = TRUE)
...
data.xf <- data |>  
  as.data.frame() |>  
  predict(TransformParams,newdata=_) |> 
  as_tibble()
...
data.imp <- data |> predict(ImputeParams,newdata=_)
